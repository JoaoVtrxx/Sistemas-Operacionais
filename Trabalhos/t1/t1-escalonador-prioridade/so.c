...
processo_t *fila_remover_prioridade(fila_t *fila);
...
static void so_escalona(so_t *self)
{
  ...
  {
    processo_t *primeiro_da_fila = fila_remover_prioridade(self->fila_prontos);
...
static void tabela_imprimir(tabela_processos_t *tabela) {
    ...
    for (int i = 0; i < tabela->num_processos; i++) {
        ...
        if (proc != NULL) {
            console_printf("|PID: %d  |  estado: %d| prioridade %f\n", proc->pid, proc->estado, proc->prioridade);
        }
    }
    console_printf("|--------------------|\n");
}
...
processo_t *fila_remover_prioridade(fila_t *fila) {
    ...
    nodo_fila_t *anterior = NULL;
    nodo_fila_t *atual = fila->inicio;
    nodo_fila_t *anterior_maior = NULL;
    nodo_fila_t *nodo_maior = fila->inicio;

  
    while (atual) {
        if (atual->processo->prioridade < nodo_maior->processo->prioridade) {
            nodo_maior = atual;         
    ...
    if (anterior_maior) {
        anterior_maior->proximo = nodo_maior->proximo;  
    } else {

        fila->inicio = nodo_maior->proximo;
    }

    if (!fila->inicio) {
        fila->fim = NULL;  
    } else if (fila->fim == nodo_maior) {
        fila->fim = anterior_maior;  
    }

    processo_t *processo = nodo_maior->processo;
    free(nodo_maior);  
    fila->tamanho--;
