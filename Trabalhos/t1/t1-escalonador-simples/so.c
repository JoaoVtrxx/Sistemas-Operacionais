...
static void so_escalona(so_t *self)
{
  ...
  if(self->processo_em_execucao != NULL){
    ...
    for(int i = 0; i < self->tabela->num_processos; i++){
      if(processo_get_estado(self->tabela->tabela_processos[i]) == PRONTO){
        if(self->processo_em_execucao != NULL){
          console_printf("SO: Escalonando processo... Sai processo %d, entra processo %d.", processo_get_pid(self->processo_em_execucao), processo_get_pid(self->tabela->tabela_processos[i]));
          
        }else{
          console_printf("SO: Escalonando processo... entra processo %d.", processo_get_pid(self->tabela->tabela_processos[i]));
          metricas_so_adiciona_tempo_ocioso(self->metricas, tempo_atual(self));
        }
        
        processo_set_estado(self->tabela->tabela_processos[i], EXECUTANDO);
        metricas_processo_incrementa_transicao(self->tabela->tabela_processos[i]->metricas, EXECUTANDO);
        metricas_processo_para_tempo_estado(self->tabela->tabela_processos[i]->metricas, self, EXECUTANDO);
        self->processo_em_execucao = self->tabela->tabela_processos[i];
        return;
      }
...
static void so_trata_irq(so_t *self, int irq)
{
  ...
  switch (irq) {
    ...
    case IRQ_RELOGIO:
      ...
      break;
    default:
      metricas_so_incrementa_interrupcoes(self->metricas, 7);
...
static void so_trata_irq_reset(so_t *self)
{
  ...
  //self->processo_em_execucao = processo_init;
...
static void so_chamada_mata_proc(so_t *self)
{
  ...
  processo_t *processo_buscado = tabela_buscar_processo(self->tabela, pid);

  if(processo_buscado != NULL){
...
static void metricas_processo_para_tempo_estado(metricas_processo_t *metricas, so_t *self, int estado){
  for(int i = 0; i < 4; i++){
    if ((i == 0 && metricas->tempo_estado[i] <= 0) || 
        (i != 0 && metricas->tempo_estado[i] < 0)) {
...
static void metricas_processo_imprimir(processo_t *processo) {
    ...
    console_printf("  - Tempo de vida: %.2f instrucoes\n", (metricas->tempo_termino - metricas->tempo_criacao));
    console_printf("  - Numero de preempções: %d\n", metricas->num_preempcoes);
    console_printf("  - Tempo medio de resposta: %.2f instrucoes\n", metricas->tempo_resposta_medio);
    console_printf("  - Tempo total nos estados:\n");
    console_printf("      * PRONTO: %.2f instrucoes\n", metricas->tempo_estado[PRONTO]);
    console_printf("      * EXECUTANDO: %.2f instrucoes\n", metricas->tempo_estado[EXECUTANDO]);
    console_printf("      * BLOQUEADO: %.2f instrucoes\n", metricas->tempo_estado[BLOQUEADO]);
    console_printf("      * MORTO: %.2f instrucoes\n", metricas->tempo_estado[MORTO]);
...
static void metricas_so_imprimir(metricas_so_t *metricas_so) {
    ...
    console_printf("  - Tempo total de execucao: %.2f instrucoes\n", metricas_so->tempo_total_execucao);
    console_printf("  - Tempo total ocioso: %.2f instrucoes\n", metricas_so->tempo_ocioso);
    ...
    for (int i = 0; i < 8; i++) {
      const char *irq_name;
      switch (i) {
            case IRQ_RESET:   irq_name = "IRQ_RESET"; break;
            case IRQ_ERR_CPU: irq_name = "IRQ_ERR_CPU"; break;
            case IRQ_SISTEMA: irq_name = "IRQ_SISTEMA"; break;
            case IRQ_RELOGIO: irq_name = "IRQ_RELOGIO"; break;
            case IRQ_TECLADO: irq_name = "IRQ_TECLADO"; break;
            case IRQ_TELA:    irq_name = "IRQ_TELA"; break;
            case N_IRQ:    irq_name = "N_IRQ"; break;
            default:          irq_name = "UNKNOWN_IRQ"; break;
      }
      
      console_printf("      * Tipo %s: %d vezes\n", irq_name, metricas_so->interrupcoes[i]);
    }
